...
class Getuser extends React.Component {
  ...
  sendrequest(to) {
    ...
    axios
      .post(`${process.env.REACT_APP_BACKEND_URL}/user/send`, data, {
        headers: {
      ...
      <div className="container-fluid">
        ...
        >
          {this.state.link.length === 0 ? (
            <center style={{ fontWeight: "bold" }}>No New Users</center>
          ) : (
            this.state.link.map((item, key) => (
              <div
                className="card"
                style={{ maxWidth: "20rem", margin: "15px", padding: "0" }}
              >
                <img
                  className="img-thumbnail mx-auto card-img-top"
                  src={
                ...
                <div className="card-body text-center">
                  <h5 className="card-title" key={key}>
                    {item.name}
                  </h5>
