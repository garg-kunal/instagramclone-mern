...
router.get("/allpost/:id", auth, (req, res) => {
    ...
    Post.find({ mainId: req.params.id }).sort('-createdAt').exec(function (err, data) {
        if (err)
            console.log(err);
        else {
            res.json({
...
router.post("/getlike", (req, res) => {
    Post.findOne({ _id: req.body.postId, 'likes.likedId': req.body.me }, (err, result) => {
        if (err)
            console.log(err);
        if (result) {
...
router.post('/unlike', (req, res) => {
    Post.findByIdAndUpdate({ _id: req.body.postId }, { $pull: { likes: { likedId: req.body.me } } }, { new: true }, (err, result) => {
        if (err)
            console.log(err)
        else {
            res.json({
...
router.put('/setlike', auth, (req, res) => {
    Post.findByIdAndUpdate({ _id: req.body.postId }, { $push: { likes: { likedId: req.body.me } } }, { new: true }, (err, result) => {
        if (err)
            console.log(err)
        else {
            res.json({
...
router.post('/comment', auth, (req, res) => {
    ...
    Post.findByIdAndUpdate({ _id: req.body.postId }, { $push: { comment: { postedBy: req.body.me, body: req.body.comment } } }, { new: true }, (err, result) => {
        if (err)
            console.log(err)
        else {
            res.json({
