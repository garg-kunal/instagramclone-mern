...
router.get("/users/:id", auth, (req, res) => {
    ...
    Profile.find({ _id: { $ne: req.params.id } }).select("-password").exec(function (err, data) {
        if (err)
            res.sendStatus(402);
        if (data) {
            res.json({
...
router.post("/follow", auth, (req, res) => {
    ...
    Profile.findByIdAndUpdate({ _id: req.body.me }, { $push: { followers: req.body.followId } }, { new: true }, (err, data) => {
        if (err)
            console.log(err)
        Profile.findByIdAndUpdate({ _id: req.body.followId }, { $push: { follow: req.body.me } }, { new: true }, (err, result) => {
            if (err)
                console.log(err);
            else {
                Request.remove({ to: req.body.me, from: req.body.followId }, (err, result) => {
                    if (err)
                        console.log(err)
                    else {
                        res.json({
